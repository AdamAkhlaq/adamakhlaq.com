...
import { isRecipeNew, isRecipeQuick } from "@/lib/recipe-types";

async function CookbookContent() {
	// Access headers to opt out of static rendering (required for new Date())
	await headers();

	const recipes = await getAllRecipes();

	// Add badge calculations
	const recipesWithBadges = recipes.map((recipe) => ({
		...recipe,
		isNew: isRecipeNew(recipe.createdAt),
		isQuick: isRecipeQuick(recipe.totalTime),
	}));
	...
	return (
		<>
			...
			<CookbookGrid initialRecipes={recipesWithBadges} />
		</>
	);
}
